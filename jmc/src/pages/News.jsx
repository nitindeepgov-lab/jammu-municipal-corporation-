...
export default function News() {
  ...
  return (
    ...
    >
      {loading ? (
        <div className="bg-white rounded-xl shadow-sm p-4 sm:p-8"><Skeleton /></div>
      ) : error || !item ? (
        <div className="bg-white rounded-xl shadow-sm p-8 sm:p-12 text-center space-y-4">
          ...
          <Link to="/" className="inline-flex items-center gap-1 text-[#003366] hover:text-[#FF6600] font-medium text-sm transition-colors">
            ← Back to Home
          </Link>
        </div>
      ) : (
        <article className="space-y-0 rounded-xl overflow-hidden shadow-md border border-gray-100">
          ...
          {imageUrl ? (
            <div className="relative h-40 sm:h-56 overflow-hidden">
              ...
              <div className="absolute bottom-0 left-0 right-0 px-3 sm:px-5 py-3 sm:py-4">
                <p className="text-[#FF9933] text-[10px] font-semibold uppercase tracking-widest mb-1">{item.release_date}</p>
                <h1 className="text-white text-sm sm:text-lg font-bold leading-snug drop-shadow">{item.title}</h1>
              </div>
            </div>
          ) : (
            <div className="relative overflow-hidden bg-gradient-to-br from-[#001f44] via-[#003366] to-[#00509e] px-4 sm:px-5 py-5 sm:py-6">
              ...
              <div className="relative z-10">
                ...
                <p className="text-[#FF9933] text-[10px] font-semibold uppercase tracking-widest mb-1.5">{item.release_date}</p>
                <h1 className="text-white text-sm sm:text-lg font-bold leading-snug max-w-2xl">{item.title}</h1>
              </div>
            </div>
          )}
          ...
          <div className="bg-white px-3 sm:px-5 py-4 sm:py-5 space-y-4 sm:space-y-5">
            ...
            {item.description ? (
              ...
              <div className="text-gray-400 text-sm italic text-center py-4">No additional description provided.</div>
            )}
            ...
            {item.link && (
              <div className="flex items-start gap-3 sm:gap-4 p-3 sm:p-4 rounded-xl bg-[#003366]/5 border border-[#003366]/10">
                ...
                <div>
                  <p className="text-xs text-gray-400 mb-1 uppercase tracking-wide font-semibold">Official Document / Reference</p>
                  <a
                    href={item.link}
                    target="_blank"
