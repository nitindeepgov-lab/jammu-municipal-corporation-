...
const ALL_STATUSES = ['All', 'Active', 'Under Evaluation', 'Awarded', 'Closed', 'Cancelled']
...
export default function SmartCityTenders() {
  ...
  const underEvaluation = tenders.filter(t => t.status === 'Under Evaluation').length
  const awardedTenders = tenders.filter(t => t.status === 'Awarded').length

  return (
    <SubpageTemplate title="Jammu Smart City Limited Tenders" breadcrumb={[{ name: 'Smart City', to: '/smart-city' }, { name: 'Tenders' }]}>
      <div className="space-y-5">

        {/* Stats Banner */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { label: 'Total Tenders', value: tenders.length, color: '#003366', bg: '#eef2f7', icon: '📋' },
            { label: 'Active', value: activeTenders, color: '#16a34a', bg: '#f0fdf4', icon: '🟢' },
            { label: 'Under Evaluation', value: underEvaluation, color: '#d97706', bg: '#fffbeb', icon: '🔍' },
            { label: 'Awarded', value: awardedTenders, color: '#2563eb', bg: '#eff6ff', icon: '✅' },
          ].map((stat, idx) => (
            <div
              key={idx}
        ...
        <div className="bg-white rounded-lg shadow-sm p-5 border border-gray-100">
          ...
          <p className="text-gray-700 text-sm leading-relaxed mb-3">
            Jammu Smart City Limited (JSCL) invites proposals and bids for various infrastructure, IT, 
            and urban development projects under the Smart Cities Mission. All tenders follow the Government 
            of India procurement guidelines and e-tendering procedures.
          </p>
        ...
        {loading && (
          <div className="bg-white rounded-lg shadow-sm p-10 text-center border border-gray-100">
            ...
            <p className="text-gray-500 text-sm">Loading tenders from server...</p>
          </div>
        )}
        ...
        {!loading && error && (
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-5">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="text-sm font-semibold text-amber-800 mb-1">Could not load tenders</h4>
                <p className="text-xs text-amber-700">Please try again later or check back shortly.</p>
              </div>
            </div>
          </div>
        )}
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 mb-3">
                      <div className="flex-1">
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          ...
                          <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full border ${STATUS_COLORS[tender.status] || STATUS_COLORS.Active}`}>
                            {tender.status}
                          </span>
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-xs text-gray-500 pt-3 border-t border-gray-100">
                      ...
                      {tender.estimatedCost && (
                        <div className="flex items-center gap-1.5 ml-auto">
                          <span className="font-semibold text-[#003366]">{tender.estimatedCost}</span>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 className="text-sm font-semibold text-[#003366] mb-1">Note</h4>
              <p className="text-xs text-gray-600 leading-relaxed">
                All Smart City tenders are published through the official J&K e-Tendering portal. 
                For detailed tender documents, eligibility criteria, and submission guidelines, 
                please visit{' '}
                <a
                ...
                Bidders are advised to regularly check the portal for corrigendum and addendum updates.
              </p>
            </div>
          </div>
        </div>
