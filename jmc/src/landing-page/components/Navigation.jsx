...
export default function Navigation({ mobileMenuOpen, setMobileMenuOpen }) {
  ...
  const subLinkClass = 'block px-5 py-3 text-slate-600 hover:text-[#003366] hover:bg-slate-50 text-[13px] font-medium transition-all duration-200 border-l-[3px] border-transparent hover:border-[#FF6600]'
  const mobileSubLinkClass = 'flex items-center px-8 py-3.5 text-slate-300 hover:text-white hover:bg-white/5 text-[13px] border-b border-white/5 last:border-0 transition-colors'
        ...
        <div className="max-w-[1300px] mx-auto px-4 lg:px-6">
          <div className="flex items-center justify-between lg:justify-start h-[52px]">
            ...
            <div className="lg:hidden flex-1 text-center">
              <p className="text-white font-bold text-[14px] leading-tight tracking-wide">Jammu Municipal Corporation</p>
              <p className="text-[#FF6600] text-[9px] uppercase tracking-[0.2em] mt-0.5">Govt. of J&K (UT)</p>
            </div>
                  ...
                  >
                    {item.href ? (
                      ...
                      >
                        {item.name}
                        {item.hasDropdown && (
                          <svg className={`w-3.5 h-3.5 ml-1.5 transition-transform duration-200 ${openDropdown === idx ? 'rotate-180 text-[#FF6600]' : 'opacity-60'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" />
                          </svg>
                        )}
                        {/* Hover accent line */}
                        <span className={`absolute bottom-0 left-0 w-full h-[3px] bg-[#FF6600] origin-left transition-transform duration-300 ease-out
                      ...
                      >
                        ...
                        {/* Hover accent line */}
                        <span className={`absolute bottom-[-2px] left-0 w-full h-[3px] bg-[#FF6600] origin-left transition-transform duration-300 ease-out
                          ${isNavActive ? 'scale-x-100' : 'scale-x-0 group-hover:scale-x-100'}`} />
                      </Link>
                    )}
      ...
      >
        ...
        <div className="bg-[#002B5E] px-5 py-5 flex items-center justify-between border-b-[3px] border-[#FF6600] flex-shrink-0 shadow-sm relative overflow-hidden">
          ...
          <div className="relative z-10">
            <p className="text-[#FF6600] text-[9px] uppercase tracking-[0.2em] font-bold mb-0.5">Govt. of J&K</p>
            <p className="text-white font-bold text-sm leading-tight">Jammu Municipal</p>
            <p className="text-slate-300 font-semibold text-xs mt-0.5">Corporation</p>
          </div>
          <button
              ...
              <li key={idx} className="border-b border-white/5 last:border-0">
                <div className="flex items-stretch min-h-[52px]">
                  {item.href ? (
                    ...
                    >
                      <span className={`mr-3 text-lg leading-none ${isNavActive ? 'text-[#FF6600]' : 'text-slate-500'}`}>·</span>
                      {item.name}
                    </Link>
                  )}
                  {item.hasDropdown && (
        ...
        <div className="bg-[#00132B] px-6 py-5 flex-shrink-0 border-t border-white/5">
          <a href="tel:18001807207" className="flex items-center gap-4 group">
            ...
            <div>
              <p className="text-white text-sm font-bold tracking-wide group-hover:text-[#FF6600] transition-colors">1800-180-7207</p>
              <p className="text-slate-400 text-[10px] uppercase font-semibold tracking-wider mt-0.5">Municipal Helpline</p>
            </div>
          </a>
        </div>
      </div>
