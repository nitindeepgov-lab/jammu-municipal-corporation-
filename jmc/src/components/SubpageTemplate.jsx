...
function MobileFloatingNav({ title }) {
  ...
  const currentPageIndex = PAGE_NAV.findIndex(
      ...
      {!open && (
        ...
        >
          ...
          <div className="flex flex-col">
            <span className="text-[11px] font-bold text-gray-800 leading-tight truncate max-w-[100px]">
              {title}
            </span>
            <span className="text-[9px] font-medium text-gray-400 leading-tight truncate max-w-[100px]">
              {currentPage.name}
            </span>
          </div>
          {/* Progress bar */}
          ...
          <div className="bg-white border border-gray-200 shadow-2xl rounded-2xl w-56 max-h-[70vh] overflow-hidden animate-fadeIn">
            ...
            <div className="px-4 py-3 border-b border-gray-100">
              <div className="flex items-center justify-between mb-1">
                <h3 className="text-[13px] font-bold text-gray-900 leading-tight truncate pr-2">
                  {title}
                </h3>
                <button
                  onClick={() => setOpen(false)}
            ...
            <div className="overflow-y-auto max-h-[55vh] py-2 px-2">
              {PAGE_NAV.map((item) => {
                ...
                return (
                  ...
                  >
                    {active && (
                      <div className="w-1 h-4 rounded-full bg-[#FF6600] shrink-0" />
                    )}
                    <Icon
                      className={`w-3.5 h-3.5 shrink-0 ${active ? "text-[#FF6600]" : "text-gray-300"}`}
                    />
      ...
      <div className="bg-white min-h-screen">
        <div className="mx-auto w-[min(96%,1400px)] px-1.5 sm:px-4 py-8 sm:py-10 md:py-14 lg:py-16">
          <div className="flex flex-col xl:flex-row gap-8 sm:gap-10 lg:gap-14 xl:gap-16">
            ...
            {showMobileFloatingNav && <MobileFloatingNav title={title} />}
